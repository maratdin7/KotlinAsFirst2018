...
class Graph {
	...
	fun getOrDefault(name: String) = try {
		get(name).neighbors
	...
	private fun bfs(start: Vertex, finish: Vertex): List<String> {
		val queue = ArrayDeque<Vertex>()
		queue.add(start)
		val visited = mutableMapOf(start to listOf<String>())
		while (queue.isNotEmpty()) {
			val next = queue.poll()
			visited[next] = visited[next]!! + next.name
			if (next == finish) return visited[finish]!!
			for (neighbor in next.neighbors) {
				if (neighbor in visited) continue
				visited[neighbor] = visited[next]!!
				queue.add(neighbor)
			}
		}
		return listOf()
